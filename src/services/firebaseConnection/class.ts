...
class Firebase {
    ...
    async getPartyByUserId(userId: string) {
        const partiesRef = collection(this.db, "parties");
        const q = query(partiesRef, where("players", "array-contains", userId));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
            const partyDocs = querySnapshot.docs.map((doc) => {
            ...
            console.log("No party found for this user.");
            return null;
        }
    }
